...
ssh -i "$VALIDATION_SSH_KEY" -o StrictHostKeyChecking=no "${VALIDATION_SSH_USER}@${VALIDATION_HOST}" \
  "VALIDATION_PDB='$VALIDATION_PDB' VALIDATION_ORDS_PORT='$VALIDATION_ORDS_PORT' VALIDATION_ORDS_DIR='$VALIDATION_ORDS_DIR' VALIDATION_ORDS_CONTAINER='$VALIDATION_ORDS_CONTAINER' BASE_ORDS_CONTAINER='$BASE_ORDS_CONTAINER' bash -se" <<'REMOTE'
set -euo pipefail
